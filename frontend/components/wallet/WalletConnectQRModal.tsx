...
export function WalletConnectQRModal({ isOpen, onClose }: WalletConnectQRModalProps) {
  ...
  const getWalletConnectUri = useCallback(async () => {
    ...
    try {
      ...
      // Start the connection process to generate URI
      // This will trigger the WalletConnect pairing flow
      const result = await connectAsync({ connector: wcConnector });

      // The URI should be available through the connector's provider
      // We need to access the internal provider to get the URI
      const provider = await (wcConnector as any).getProvider();

      // Try multiple ways to get the URI
      let uri = provider?.signer?.session?.uri
        || provider?.session?.uri
        || (wcConnector as any)?.uri
        || provider?.uri;

      // If still no URI, check if we can get it from the connector directly
      if (!uri && typeof (wcConnector as any).getProvider === 'function') {
        // The provider should emit events with the URI
      ...
      if (uri) {
        ...
        // Set loading to false and let the event handler catch the URI
        setIsLoading(false);
      }
            ...
            <div className="flex flex-col items-center">
              ...
              {qrUri ? (
                ...
                <div className="w-64 h-64 bg-gray-100 rounded-xl border-2 border-gray-200 flex items-center justify-center">
                  <p className="text-gray-500 text-center px-4">Waiting for QR code...</p>
                </div>
              )}
